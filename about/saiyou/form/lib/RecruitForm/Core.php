<?php
namespace RecruitForm;

// Modelの読み込み
require_once(__DIR__ . '/Model/Base.php');

use RecruitForm\Model;
...
class Core {
  ...
  public function getData() {
    ...
    foreach ($data['interfaces'] as $key => $options) {
      ...
      if (isset($options['pattern'])) {
        $options['pattern'] = preg_replace("/^.|.$/", '', $options['pattern']);
      }
  ...
  public function saveCSV($id, $data = []) {
    ...
    $headers = ['ID', '投稿日時'];
    $keys = [];
    foreach ($this->config['model'] as $key => $model) {
      if ($key !== 'token' && $key !== 'agreement') {
        $keys[] = $key;
        $headers[] = $model['label'];
      }
    }
    ...
    if ($mode === 'index') {
      ...
      if (!$this->validateToken($_POST['token'])) {
        ...
        if (count($data['errors'])) {
          ...
          // ファイルの一時アップロード処理
          $tempFileNames = $this->saveFileTempAll();
          foreach ($tempFileNames as $name => $tempFileName) {
            $data['params'][$name] = $tempFileName;
          }
          ...
          // ファイルとCSVデータを保存
          $id = $this->getIdFromCSV();
          $filenames = $this->saveFileAll($id, $data['params']);
          foreach ($filenames as $name => $filename) {
            $data['params'][$name] = $filename;
          }
