	...
	return function(editor) {
		...
		function appendStylesToData(dom, data, elm) {
			...
			if (css["border-color"]) {
				...
				data.borderColor = css["border"].substr(css["border"].indexOf('#'), 7);
			}
		...
		self.table = function(isProps) {
			...
			function onSubmitTableForm() {
				...
				Tools.each('backgroundColor borderColor backgroundImage'.split(' '), function(name) {
					delete data[name];
				});
				...
				editor.undoManager.transact(function() {
					...
					editor.dom.setAttribs(tableElm, {
						...
						border: data.border,
						style: data.style.replace("url('ここに取得した画像URLをペースト')", ""),
						'class': data['class']
					});

					// Border style and width add
